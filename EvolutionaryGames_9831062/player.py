...
class Player():
    ...
    def __init__(self, mode, control=False):
        ...
        self.layer_sizes = self.init_network(mode)
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        if mode == 'gravity' or mode == 'helicopter':
            x.append(velocity / 10)

            if len(box_lists) >= 1:
                x.append((box_lists[0].x - agent_position[0]) / CONFIG['WIDTH'])
                x.append((box_lists[0].gap_mid - agent_position[1]) / CONFIG['HEIGHT'])

            if len(box_lists) >= 2:
                x.append((box_lists[1].x - agent_position[0]) / CONFIG['WIDTH'])
                x.append((box_lists[1].gap_mid - agent_position[1]) / CONFIG['HEIGHT'])

            for _ in range (self.layer_sizes[0] - len(x)):
                x.append(0)

            x = np.reshape(x, (-1, 1))

            res = self.nn.forward(x)
            direction = 1 if res > 0.5 else -1
        else:
            x.append(velocity / 100)
            ...
            res = self.nn.forward(x)
            direction = 1 if res > 0.35 else 0 if res > 0.65 else -1
